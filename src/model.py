...
class Model:
    ...
    def evaluate(self, data):
        ...
        # for batch in data[:self.args.eval_subset]:
            ...
            with torch.no_grad():
                ...
                question_ids.extend(batch['text1_id'])
                answer_ids.extend(batch['text2_id'])
        outputs = {
            'target': targets,
            'prob': probabilities,
            'pred': predictions,
    ...
    def process_data(self, batch):
        # context = torch.LongTensor(batch['context']).to(self.device)
        query = torch.LongTensor(batch['text1']).to(self.device)
        response = torch.LongTensor(batch['text2']).to(self.device)
        # context_mask = torch.ne(context, self.args.padding).unsqueeze(2)
        query_mask = torch.ne(query, self.args.padding).unsqueeze(2)
        response_mask = torch.ne(response, self.args.padding).unsqueeze(2)
        inputs = {
            'text1': query,
            'text2': response,
            'text1_mask': query_mask,
            'text2_mask': response_mask,
        }
        if 'target' in batch:
